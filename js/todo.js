...
function pomeriTask(index){
   ...
   if(trenutnitask.status==="planirano"){
    ...
    trenutnitask.status="zavrseno";
       ...
       alert("Ovaj task je vec zavrsen");
   }
   razvrstajTaskove();
}

function obrisiTask(index){
    if(confirm,"Da li ste sigurni da zelite da obrisete ovaj task?"){
...
divZavrseno.innerHTML="";
  ...
  taskovi.forEach((task,index) => {
    ...
    taskElement.innerHTML=`<h3>${task.naslov}</h3>
    <p>${task.opis}</p>
    <p>${task.datum}</p>
